...
// Key buffers
//             上一轮去抖后      去抖临时用                 实时的
static uint8_t keys[COLUMNS+2], keys_snapshot[COLUMNS+2], keys_buffer[COLUMNS+2];
static uint32_t debounce_ticks, activity_ticks;
static volatile bool debouncing = false;

static void send_mx_data(void) {
    tx_payload.length = COLUMNS+2;
    memcpy(tx_payload.data, keys, COLUMNS+2);
...
bool compare_keys(uint8_t* first, uint8_t* second, uint32_t size) {
